...
//    println(portLocation1)
        ...
        val portLocation2 =
            dataProductService.retrieveNumValidPortsSafe(DataProductId(dataProductId))
        println(portLocation2)

        val id1: Long = 1
        val id2: Long = 32
        val combinedLocations =
            dataProductService.combineLocationsSafely(DataProductId(id1), DataProductId(id2))
        println(combinedLocations)

        val combinedLocations2 =
            dataProductService.combineLocationsSafelyArrow(DataProductId(id1), DataProductId(id2))
        println(combinedLocations2)

        val combinedLocations3 =
            nullable {
                dataProductService.combineLocationsUsingRaiseContextNullable(
                    DataProductId(
                        id1
                    ), DataProductId(id2)
                )
        ...
        val combinedLocations4 =
            either {
                dataProductService.combineLocationsUsingRaiseContextEither(
                    DataProductId(id1),
                    DataProductId(id2)
                )
            }
        ...
        val maxNumValidPortsGap =
            dataProductService.getNumValidPortsGapWithMaxOption(DataProductId(id2))
        ...
        val maxNumValidPortsGap3 =
            dataProductService.getNumValidPortsGapWithMaxEither(DataProductId(id2))
        ...
        val invalidDataProduct2 = DataProduct(
            ...
            listOf(
                Port(
                    PortId(1),
                    PortName(" "),
                    PortLocation("Hive: db.table"),
                    PortAvailability(-1.0)
                ),
                Port(PortId(2), PortName("Port 2"), PortLocation(" "), PortAvailability(98.0))
            )
        )

        fun printValidationResult(dataProduct: DataProduct) {
            when (val validated = dataProductService.validateDataProduct(dataProduct)) {
                is Either.Left -> println(
                    "Validation failed: ${
                        validated.value.all.joinToString(
                            "\n",
                            "[\n",
                            "\n]",
                ...
                is Either.Right -> println("Validation succeeded")
            }
        }

        printValidationResult(validDataProduct)
        printValidationResult(invalidDataProduct1)
        printValidationResult(invalidDataProduct2)
    }
}